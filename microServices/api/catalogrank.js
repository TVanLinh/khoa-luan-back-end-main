...
module.exports = {
    get_index: function () {
        ...
        //
        //  CatalogRank.insertMany(a2,function(err){
        //
        // });
        // CatalogRank.remove({name:'ok'}, err => {
        //
        // });
        return CatalogRank.find();
    },
    u_post_index: function (data) {
        return CatalogRank.findOne({name: data['name']}).then(result => {
            if (result && result.name) {
                return {
                    msg: "Loại này đã tồn tại rồi "
                };
            } else {
                return CatalogRank(data).save();
            }
        });
    },
    u_put_index: function (data) {
        console.log(JSON.stringify(data));
        return CatalogRank.findByIdAndUpdate(data._id, data);
    },
    u_delete_index: function (id) {
        return CatalogRank.findByIdAndRemove(id);
    }
};
